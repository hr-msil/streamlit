...
from chequeo_decretos import chequear_decreto_oficina_empleado
from chequeo_decretos import listadoPorEmpleados

st.title("📝 PRODUCTIVIDADES")

st.divider()

tab1,tab2,tab3,tab4 = st.tabs(["Subir archivos", "Sin comparar","Resultados","Chequeo de decreto por oficina"])
...
with tab4:
    st.write("Subí los decretos que quieras chequear")
    archivos_csv_decreto = st.file_uploader("Seleccionar archivo", type = "csv", key = "decreto_chequeo",accept_multiple_files=True)
    st.write("Subí el listado de empleados por oficina del sistema")
    empleados_por_ofi_excel = st.file_uploader("Seleccionar archivo", type = "xls",key = "empl_oficina",accept_multiple_files=False)
    st.write("Subí la tabla de decretos por oficina")
    decreto_por_ofi_excel = st.file_uploader("Seleccionar archivo", type="xlsx",key = "decreto_por_ofi",accept_multiple_files=False)
    
    
    if archivos_csv_decreto and empleados_por_ofi_excel and decreto_por_ofi_excel:
        df_empleados_por_ofi = listadoPorEmpleados(empleados_por_ofi_excel)
        decreto_por_ofi = pd.read_excel(decreto_por_ofi_excel)
        for archivo_csv_decreto in archivos_csv_decreto:
            decreto = archivo_csv_decreto.name.split(".")[0]
            df_csv = lectura_archivo_dec(archivo_csv_decreto,decreto, decreto)
            legajos_no_encontrados = chequear_decreto_oficina_empleado(df_empleados_por_ofi,archivo_csv_decreto,decreto_por_ofi,df_csv)
            st.markdown(f"Resultados para el decreto: {decreto}")
            if(len(legajos_no_encontrados) > 0):
                st.info(f"Estos son los legajos que no fueron encontrados en el listado por empleados, o no corresponden cobrar el decreto {archivo_csv_decreto.name}")
                for legajo in legajos_no_encontrados:
                    st.info(legajo)
            else: 
                st.info(f"No se encontraron inconsistencias")
                st.balloons()
